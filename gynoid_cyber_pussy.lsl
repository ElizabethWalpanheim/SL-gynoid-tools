...
set_hole_size(integer no, float y_hole)
{
    ...
    llWhisper(CyberVaginaChannel,"HUD vag "+(string)no+" "+(string)y_hole);
...
makeHoney(key id, integer follow)
{
    integer i;
    for (i=0; i<llGetListLength(friends); i++)
        if (llList2Key(friends,i) == id) {
...
fck_touch(key fid)
{
    ...
    if (sense >= critical_sense) orgazzm(1);
        ...
        if (sense == 0) {
            ...
            if ((fid != NULL_KEY) && (fid != owner))
                // send IM only for first one due to a big time of IM generation
                llInstantMessage(fid,"You've touched me right now!\nUhhmmmm......");
        }
        myHoney();
...
default
{
    ...
    timer()
    {
        if (stat == 0) {
            ...
            if (c_minutes >= minutes_to_hurt) {
                ...
                c_last_charge -= 2;
                llWhisper(GynoidIChan,"PWR "+(string)c_last_charge);
                if ((c_minutes > (minutes_to_hurt * 2)) && (llFrand(1.0) < 0.5)) {
                    llOwnerSay("Ooops...");
                    power(1);
                }
            }
    ...
    touch_start(integer tn)
    {
        ...
        for (i=0; i<tn; i++) {
            ...
            if (i == 0)
                makeHoney(llDetectedKey(i),1);
            else
                makeHoney(llDetectedKey(i),0);
        }
        c_touches += tn;
        fck_touch(llDetectedKey(0));
    }
    ...
    listen(integer ch, string nm, key id, string msg)
    {
        ...
        if ( (ch == VaginaPubChannel) && ((id == owner) || (llGetOwnerKey(id) == owner)) ) {
            ...
            else if (msg == "reset") { llMessageLinked(LINK_ALL_OTHERS,0,"reset",NULL_KEY); llResetScript(); }
            ...
            else if (msg == "touch") fck_touch(owner);
            else if (msg == "mem") llOwnerSay("Memory status: "+(string)llGetFreeMemory()+" bytes free.");
            else if (msg == "help") {
                llOwnerSay("Commands available:");
                llOwnerSay("on off lock unlock pee cum fire reset open close cycle hide show mem");
            }
        } else if ((ch == GynoidIChan) && (llGetOwnerKey(id) == owner)) {
            if (llListFindList(llParseString2List(nm,[],["battery"]),["battery"]) >= 0)
                c_last_charge = (integer)msg; // TODO: make this more stable and less ugly =)
            else if (llGetSubString(msg,0,6) == "VAGINA!") {
                // extended commands from brain
                list l = llParseString2List(msg,["|"],[]);
                string cmd = llList2String(l,1);
//                llOwnerSay("vagina debug: received command "+cmd);
                if (cmd == "FRESET") {
                    llMessageLinked(LINK_ALL_OTHERS,0,"reset",NULL_KEY);
                    llResetScript();
                } else if (cmd == "UPD") {
                    ...
                    rcontrols((integer)llList2String(l,2));
                } else if (cmd == "SELFTOUCH") {
                    fck_touch(owner);
                }
            }
        }
